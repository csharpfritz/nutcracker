...
public class LightshowService(LedService ledService, ILogger<LightshowService> logger, IWebHostEnvironment environment) : BackgroundService
{
	...
	/// </summary>
	public int GetVolume() => _currentVolume;
	...
	[
		new("Santa Claus Is Comin To Town", new TimeSpan(0, 4, 28), "music/Bruce Springsteen - Santa Claus Is Comin To Town (Official Audio).mp3", "lights/bruce-springsteen-santa-claus.json"),
		new("Deck the Halls", new TimeSpan(0, 1, 14), "music/deck-the-halls-christmas-bells-129141.mp3", "lights/deck-the-halls.json"),
		new("Grandma Got Run Over By A Reindeer", new TimeSpan(0, 3, 24), "music/Elmo & Patsy - Grandma Got Run Over By A Reindeer.mp3", "lights/grandma-got-run-over.json"),
		new("Rudolph the Red-Nosed Reindeer", new TimeSpan(0, 3, 8), "music/Gene Autry - Rudolph the Red-Nosed Reindeer (Audio).mp3", "lights/rudolph.json"),
		new("Hark the Herald Angels Sing", new TimeSpan(0, 4, 41), "music/hark-the-herald-angels-sing-celtic-christmas-411990.mp3", "lights/hark-the-herald.json"),
		new("Joy to the World", new TimeSpan(0, 2, 3), "music/joy-to-the-world-xmas-house-background-music-for-video-full-version-431806.mp3", "lights/joy-to-the-world.json"),
		new("O Come All Ye Faithful", new TimeSpan(0, 5, 22), "music/o-come-all-ye-faithful-celtic-christmas-411995.mp3", "lights/o-come-all-ye-faithful.json"),
		new("Oh Christmas Tree", new TimeSpan(0, 1, 41), "music/oh-christmas-tree-168949.mp3", "lights/oh-christmas-tree.json"),
		new("Santa Claus Is Coming to Town (Piano)", new TimeSpan(0, 1, 49), "music/santa-claus-is-coming-to-town-christmas-piano-248543.mp3", "lights/santa-claus-piano.json"),
		new("Silent Night", new TimeSpan(0, 2, 51), "music/silent-night-acoustic-indie-version-127052.mp3", "lights/silent-night.json"),
		new("Wizards in Winter", new TimeSpan(0, 3, 5), "music/trans-siberian-orchestra-wizards-in-winter.mp3", "lights/wizards-in-winter.json")
	];
		...
		while (!stoppingToken.IsCancellationRequested)
		{
			if (LightshowQueue.TryDequeue(out var lightshowSettings))
			{
				...
				try
				{
					...
					var musicFilePath = Path.Combine(environment.WebRootPath, lightshowSettings.MusicFilePath);
					var musicProcess = new System.Diagnostics.Process
					{
						StartInfo = new System.Diagnostics.ProcessStartInfo
						{
							FileName = "mplayer",
							Arguments = $"-volume {_currentVolume} -ao pulse -really-quiet -nolirc \"{musicFilePath}\"",
							UseShellExecute = false,
							CreateNoWindow = true,
							RedirectStandardOutput = true,
							RedirectStandardError = true,
